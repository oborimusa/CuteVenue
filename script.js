// ============================================================
// CuteVenue — Complete JavaScript (Event Halls Focus)
// ============================================================
...
let selectedAddons = {
    catering: 'none',
    decor: 'none',
    sound: 'none' // Changed from 'lodging' to 'sound'
};

const addonPrices = {
    catering: { none: 0, basic: 15000, premium: 25000, luxury: 40000 },
    decor:    { none: 0, basic: 10000, premium: 20000, luxury: 35000 },
    sound:    { none: 0, basic: 20000, premium: 40000, luxury: 60000 } // Changed from lodging
};
...
async function loadVenues() {
    try {
        ...
        console.log("✅ Event Halls loaded:", venues.length);
        return venues;
    } catch (error) {
        console.error("❌ Error loading event halls:", error);
        return [];
    }
}
...
function renderVenueDropdown() {
    ...
    let html = `<label for="venueSelect">Select an Event Hall:</label>`;
    html += `<select id="venueSelect" onchange="onVenueChange(this.value)">`;
    html += `<option value="">-- Select a hall --</option>`;

    const filtered = selectedVenueType === 'all' ? venues : venues.filter(v => v.type === selectedVenueType);

    filtered.forEach(venue => {
        html += `<option value="${venue.id}">${venue.name} (${venue.type || 'event hall'})</option>`;
    });
    html += `</select>`;
...
function renderVenues(venuesList = null) {
    ...
    if (filtered.length === 0) {
        grid.innerHTML = `<p style="color: #888; text-align: center;">No event halls found for this category. Please try another.</p>`;
        return;
    }
    ...
    grid.innerHTML = filtered.map(venue => `
        <div class="venue-card" data-id="${venue.id}" onclick="selectVenue('${venue.id}')">
            ...
            <h4>${venue.name || "Event Hall"}</h4>
            <p class="venue-type">${venue.type || 'event hall'}</p>
...
function getVenueIcon(type) {
    const icons = {
        ...
        wedding: '💒',
        conference: '💼',
        default: '🏛️' // Default to event hall icon
    };
...
function renderSeating() {
    ...
    if (!selectedVenue) {
        view.innerHTML = '<p style="color: #888;">Select a hall to view details and seating layout.</p>';
        return;
    }
    ...
    let html = `<h4>${selectedVenue.name} — Seating Layout</h4>`;
...
function generateSummary() {
    ...
    if (!selectedVenue) {
        document.getElementById('bookingSummary').innerHTML = `
            <p style="color: #ff4444;">⚠️ Please select an event hall in Step 2.</p>
        `;
        return;
    }
    ...
    const soundDesc = document.querySelector('#sound-description textarea')?.value || 'None';
    ...
    const snd = addonPrices.sound[selectedAddons.sound] || 0;

    const venuePrice = parseInt(selectedVenue.price || 0);
    const total = venuePrice + cat + dec + snd;
    const deposit = total * 0.5;

    bookingData = {
        venueId: selectedVenue.id,
        venueName: selectedVenue.name,
        venueType: selectedVenue.type || 'event hall',
        date,
        eventType,
        addons: { ...selectedAddons },
        addonDescriptions: { catering: cateringDesc, decor: decorDesc, sound: soundDesc },
        globalNotes,
        total,
        deposit
    };
    ...
    summary.innerHTML = `
        ...
        <p><strong>Event Hall:</strong> ${selectedVenue.name}</p>
        <p><strong>Type:</strong> ${selectedVenue.type || 'event hall'}</p>
        ...
        <p><strong>Sound & Lighting:</strong> ${selectedAddons.sound} — N${snd.toLocaleString()}</p>
        ${selectedAddons.sound === 'none' ? `<p style="color: #888; font-size: 0.85rem;"><em>Description: ${soundDesc}</em></p>` : ''}
    ...
    try {
        ...
        content.innerHTML = `
            <div style="padding: 20px; border: 1px solid #333; border-radius: 12px;">
                ...
                <p><strong>Event Hall:</strong> ${b.venueName}</p>
...
function sendReminder() {
    alert('📧 Reminder sent to customer! (Email service integration required)');
}
...
console.log("📍 CuteVenue (Event Halls Focus) loaded successfully!");
// ============================================================
// FLOATING WHATSAPP BUTTON (Auto-inject)
// ============================================================
...
(function addWhatsAppButton() {
    ...
    btn.href = 'https://wa.me/2348039661354?text=Hi%20CuteVenue%2C%20I%20want%20to%20book%20an%20event%20hall';
    btn.target = '_blank';
    btn.rel = 'noopener noreferrer';
    btn.innerHTML = '<i class="fab fa-whatsapp" style="font-size: 35px;"></i>';
    btn.style.cssText = `
        position: fixed;
        bottom: 30px;
        right: 30px;
    ...
    btn.addEventListener('mouseover', () => {
        ...
        btn.style.boxShadow = '0 12px 35px rgba(37, 211, 102, 0.6)';
    });
    btn.addEventListener('mouseout', () => {
        btn.style.transform = 'scale(1)';
        btn.style.boxShadow = '0 8px 25px rgba(37, 211, 102, 0.4)';
    });
